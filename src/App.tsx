  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      edges.forEach(e => {
        ...
        let a = 0.45 + (0.95 - 0.45) * e.emphasis;
        let lw = 1.2 + (3.5 - 1.2) * e.emphasis;
        let sb = 15 * e.emphasis;
        ...
        ctx.shadowColor = `rgba(37, 99, 235, ${0.6 * e.emphasis})`;
        ctx.shadowBlur = sb;
        ctx.stroke();

        // Highlight trace animation
        if (e.emphasis > 0.01) {
          ctx.save();
          ctx.beginPath();
          ctx.moveTo(s.x, s.y);
          ctx.quadraticCurveTo(cpX, cpY, t.x, t.y);
          ctx.strokeStyle = `rgba(255, 255, 255, ${0.8 * e.emphasis})`;
